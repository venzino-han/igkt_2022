...
def main():
    ...
    for f in file_list:
        ...
        if args.model_type == 'IGMC':
            knowledge_concept=False
        else: knowledge_concept=True
        ...
        for lr in args.train_lrs:
            ...
            best_auc = train(sub_args, train_loader, test_loader, logger=logger)
            best_auc_list.append(best_auc)
        
        logger.info(f"**********The final best testing RMSE is {min(best_auc_list):.6f} at lr {best_lr}********")
        logger.info(f"**********The mean testing RMSE is {np.mean(best_auc_list):.6f}, {np.std(best_auc_list)} ********")
